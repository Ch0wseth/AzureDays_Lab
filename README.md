...
À la fin de la démo : `Get-Content copilot-otel.jsonl | Select-String tokens` (ou `cat ... | grep tokens`).
...
3. Ouvrir le fichier avec `Get-Content src/utils/validators.js` (ou `cat`).
...
→ `playwright` et `awesome-copilot` listés.
...
# 3. Démarrer la démo (autre terminal)
$env:COPILOT_OTEL_ENABLED = "true"
$env:COPILOT_OTEL_FILE_EXPORTER_PATH = "./copilot-otel.jsonl"
copilot
